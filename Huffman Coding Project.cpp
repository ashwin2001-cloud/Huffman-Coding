...
class BinaryTree{
 ...
 public:
     ...
     //below are the functions corresponding to binary tree class
     void print();
...
// heap is built
    ...
    while (Heap.T[0].root->freq>1){
        ...
        //huffmanTree is the node to be inserted
        // huffmantree is already declared in HuffmanCode class as a binaryTree
      ...
      // T[1] is when the frequenct of tree becomes 1
      ...
      cout<<"\nEnter the string to be encoded by Huffman Coding: ";
      char *str;
      str=new char[60];
      cin>>str;
...
T= new BinaryTree [n+1];
    ...
    for (int i=1; i<=n; i++){
        ...
        cin>>T[i].root->info;
        cout<<"Frequency of the character:- ";
        cin>>T[i].root->freq;
        T[i].root->code=NULL;
        T[i].root->Llink=NULL;
        T[i].root->Rlink=NULL;
...
void minHeap::heapify(int i){
    ...
    // base call
        ...
        // left child node has the lowest frequency    
        if(T[2*i].root->freq <= T[2*i+1].root->freq){
            swap(T[2*i],T[i]);
                i=2*i;
        }
       
        // right child node has the lowest frequency
        else{
   swap(T[2*i+1],T[i]);
            i=2*i+1;
        }    
...
// base case
// which is of no use as such condition will never exist
    if(root==NULL)
        return;  
    ...
    //l.root stores address of left child node
    //same for right child node
    l.root=root->Llink;
    r.root=root->Rlink;
    l.root->code=new char[i+1];
    r.root->code=new char[i+1];
   
    //storing code in, 0 to i-1 indices, same as that of the parent
    for (int k=0; k<i; k++){
...
//flag
//when we reach the required leaf node, flag becomes 1          
int f=0;
